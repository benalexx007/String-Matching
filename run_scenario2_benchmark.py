...
EXE_PATH = ROOT / f"crossword_benchmark_s2_{os.getpid()}.exe"
CSV_PATH = ROOT / "Số liệu - Scenario 2.csv"
INPUT_ROOT = ROOT / "Benchmark Test" / "Input" / "Scenario 2"
OUTPUT_ROOT = ROOT / "Benchmark Test" / "Output" / "Scenario 2"

DICT_SIZES = [10, 50, 100, 200]
MODES = ["random", "spiral"]
ALGORITHMS = [
    ("bf", "Brute force"),
    ("kmp", "KMP"),
...
def parse_metrics(output_file: Path):
    ...
    return float(time_match.group(1)), int(cmp_match.group(1))


def run_all_cases(selected_algorithms, selected_modes, selected_dict_sizes):
    metrics = {}
    for algo_key, _ in ALGORITHMS:
        ...
        for mode in MODES:
            if mode not in selected_modes:
                continue
            for k in DICT_SIZES:
                if k not in selected_dict_sizes:
                    continue
                input_path = INPUT_ROOT / str(k) / f"{mode}.txt"
                output_path = OUTPUT_ROOT / algo_key / str(k) / f"{mode}.txt"
                if not input_path.exists():
                    raise FileNotFoundError(f"Missing input file: {input_path}")
                output_path.parent.mkdir(parents=True, exist_ok=True)
                args = [str(EXE_PATH), "-a", algo_key, "-i", str(input_path), "-o", str(output_path)]
                print(f"Running {algo_key.upper()} | {mode:6} | K={k}", flush=True)
                run_cmd(args)
                metrics[(algo_key, mode, k)] = parse_metrics(output_path)
    return metrics
...
def write_csv(metrics):
    ...
    for algo_key, algo_label in ALGORITHMS:
        for i, mode in enumerate(MODES):
            ...
            for k in DICT_SIZES:
                if (algo_key, mode, k) in metrics:
                    t, c = metrics[(algo_key, mode, k)]
                    row.extend([f"{t:.3f}", str(c)])
                else:
                    row.extend(["", ""])
            rows.append(row)
...
def main():
    try:
        parser = argparse.ArgumentParser(description="Run Scenario 2 benchmark and fill CSV.")
        ...
        parser.add_argument("--dict-sizes", nargs="*", type=int, default=DICT_SIZES, help="Dictionary sizes: 10 50 100 200")
        parser.add_argument("--skip-csv", action="store_true", help="Run cases but do not rewrite CSV")
        args = parser.parse_args()

        valid_algos = {k for k, _ in ALGORITHMS}
        for a in args.algos:
        ...
        for k in args.dict_sizes:
            if k not in DICT_SIZES:
                raise ValueError(f"Unsupported dictionary size: {k}")

        compile_executable()
        metrics = run_all_cases(args.algos, args.modes, args.dict_sizes)
